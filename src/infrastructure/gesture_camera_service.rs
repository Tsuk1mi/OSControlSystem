...
use crate::gesture_os_control::adapters::input::webcam_adapter::{WebcamAdapter, WebcamAdapterConfig};
...
use crate::gesture_os_control::domain::entities::gesture::{AppRunMode, FrameProcessingOutcome, PipelineGestureStats};
...
pub enum GestureServiceMessage {
    ...
    PreviewFrame { width: u32, height: u32, rgb8: Vec<u8> },
...
impl GestureRecognitionService for WebcamGestureService {
    ...
    fn start(
        ...
        let handle = thread::spawn(move || {
            if let Err(error) = pipeline_worker(sen_arc, cd_arc, camera, message_tx.clone(), stop_rx) {
...
fn pipeline_worker(
    ...
    loop {
        if stop_rx.try_recv().is_ok() {
            let _ = message_tx.send(GestureServiceMessage::Status("Камера остановлена.".to_owned()));
            adapter.close();
            return Ok(());
        }
        ...
        match adapter.read_frame() {
            Ok(frame) => {
                ...
                if last_stats_sent.elapsed() >= Duration::from_millis(180) {
                    ...
                    let _ = message_tx.send(GestureServiceMessage::PipelineStats(session.stats.clone()));
                }
                ...
                match result.outcome {
                    FrameProcessingOutcome::CommandReady { command } => {
                        ...
                        thread::spawn(move || {
                            ...
                            let line = if exec.ok {
                                format!("OK {} · {trigger}", command.label_ru())
                            } else {
                                format!(
                                    "нет {}: {}",
                                    command.label_ru(),
